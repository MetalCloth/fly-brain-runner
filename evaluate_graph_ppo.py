"""Evaluate the reduced fly-inspired graph policy on fixed seeds."""
...
from fly_graph import FlyGraphExtractor  # noqa: F401 - needed when loading the model
from gym_env import PartialPixelGymRunnerEnv


def evaluate(model_path: Path, episodes: int, max_steps: int) -> list[dict[str, float]]:
    env = DummyVecEnv(
        [lambda: PartialPixelGymRunnerEnv(max_steps=max_steps, spawn_interval=8)]
    )
